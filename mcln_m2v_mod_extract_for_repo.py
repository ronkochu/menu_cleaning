...
def extract_full_tier(out_df, params, tier, fdir, model_names):
	...
	for label in out_df['prediction_%s' % (tier-1)].unique():
		...
		for pvl in prev_tier_labels:
			...
			if len(tf_input) > 0:
				_, preds = bag_model_predictions(all_stim[label_idx], fdir, model_names, prev_tier_label);
				max_idx = preds.argmax(axis = 1);
				max_vals = np.amax(preds, axis = 1);
				
				pred_cols[label_idx,0] = tf_input['unique_labels'][max_idx];
...
def bag_model_predictions(all_stim, fdir, model_names, prev_tier_label):
	...
	for model_name in model_names:
		...
		if os.path.isfile('%s_pt2.meta' % savename):
			#print('updated model found for %s' % prev_tier_label)
			savename = '%s_pt2' % savename;
...
def extract_tier_subset(all_stim, params, tf_input, savename):
	...
	if 'token' in savename:
		all_preds = m2v.get_tf_prediction_pvals(all_stim.tolist(), sess, params, tf_vars, 'tokens');
	else:
		all_preds = m2v.get_tf_prediction_pvals(all_stim.tolist(), sess, params, tf_vars, 'chars');
...
def extraction_block(df, fdir, model_names, params):
	...
	out_df['prediction_1'] = tf_input['unique_labels'][max_idx];
...
if __name__ == "__main__":
	...
	model_names = [
		'menu2vec_token_subset_20180626',
		'menu2vec_char_subset_20180626',
	];
	out_path = '%s_labelled.csv' % in_path.split('.csv')[0]
	...
	out_df = out_df.rename(columns = {
		'prediction_1': 'CLEANSED_TIER_1',
		'prediction_2': 'CLEANSED_TIER_2',
		'prediction_3': 'CLEANSED_TIER_3',
		'prediction_4': 'CLEANSED_TIER_4',
	});
	out_df = out_df[[in_col, 'CLEANSED_TIER_1', 'CLEANSED_TIER_2', 'CLEANSED_TIER_3', 'CLEANSED_TIER_4']];
